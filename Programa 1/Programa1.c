...
int removeStudent(list *lista, FILE *fileToEdit, int auxMat)
{
    ...
    default:
        ...
        for (node *I = lista->inicio; I != NULL; prev = I, I = I->proximo)
        {
            ...
            if (I->aluno.matricula == auxMat)
            { // Encontrou algum nó com a matrícula igual à digitada pelo usuário.
                if (lista->inicio == lista->fim)
                { // Verifica se a lista possui apenas um Aluno (Nó).
                    // Inicializa a lista como vazia.
                    lista->inicio = NULL;
                    lista->fim = NULL;
                }
                else
                { // se a lista possuir mais de um aluno (Nó).
